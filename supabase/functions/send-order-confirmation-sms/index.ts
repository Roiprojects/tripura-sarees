// Send "order placed" confirmation SMS to the customer via Nettyfish.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: adminRole } = await admin
      ...
      .eq("role", "admin")
      .maybeSingle();
    const isOwner = order.user_id === caller.id;
    ...
    const text =
      `Thank you for shopping with ${STORE_NAME}. Your order ${shortId} has been successfully placed. ` +
